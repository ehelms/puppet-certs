...
describe 'certs' do
  ...
  context 'with default params' do
    ...
    describe command("certutil -L -d #{nssdb_dir} -n ca") do
      its(:exit_status) { should eq 0 }
      its(:stdout) { should match(/        Subject: "CN=#{host_inventory['fqdn']},OU=SomeOrgUnit,O=Katello,L=Raleig
            h,ST=North Carolina,C=US"/) }
      its(:stdout) { should match(/        Issuer: "CN=#{host_inventory['fqdn']},OU=SomeOrgUnit,O=Katello,L=Raleigh
            ,ST=North Carolina,C=US"/) }
    end

    describe command("certutil -L -d #{nssdb_dir} -n broker") do
      its(:exit_status) { should eq 0 }
      its(:stdout) { should match(/        Subject: "CN=#{host_inventory['fqdn']},OU=SomeOrgUnit,O=pulp,ST=North Ca
            rolina,C=US"/) }
      its(:stdout) { should match(/        Issuer: "CN=#{host_inventory['fqdn']},OU=SomeOrgUnit,O=Katello,L=Raleigh
            ,ST=North Carolina,C=US"/) }
    end
